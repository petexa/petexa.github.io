...
async function loadEvents() {
  try {
    ...
    events = results.filter(result => result.status === 'fulfilled').map(result => result.value);
  ...
  upcomingEvents.forEach((ev, i) => {
    ...
    if (shouldShowMoreInfo) {
      ...
      moreInfoBtn.setAttribute(
        'aria-label',
        `More information about ${ev.name} (opens in new tab)`
      );
  ...
  eventCards.forEach(card => {
    ...
    if (days < 2) {
      ...
      if (days === 1) {
        ...
        countdownEl.innerHTML = `<b>${hours}</b> hour${
          hours !== 1 ? 's' : ''
        }, <b>${minutes}</b> min${minutes !== 1 ? 's' : ''} left`;
      } else {
