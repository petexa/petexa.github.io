...
bad_instructions = workouts[~workouts['Instructions'].str.contains(r"\b(run|row|pull|push|clean|snatch|thruster|burpee|squat|swing|jump|bike)\b", case=False, na=False)]
if not bad_instructions.empty:
    errors.append('Some workouts may have malformed instructions; sample names: ' + ', '.join(bad_instructions['Name'].head(5).tolist()))
...
with open(report, 'w') as f:
    if errors:
        ...
        f.write('Rows: Workouts=%d Movements=%d WM=%d ME=%d Equipment=%d\n' % (len(workouts), len(movements), len(wm), len(me), len(equipment)))
