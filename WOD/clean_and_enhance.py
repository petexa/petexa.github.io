...
class WODCleaner:
    ...
    def remove_movement_artifacts(self):
        ...
        if artifacts_to_remove:
            ...
            if wm_removed > 0:
                self.changes.append(f"Removed {wm_removed} workout-movement mappings for artifact movements")
            ...
            if me_removed > 0:
                self.changes.append(f"Removed {me_removed} movement-equipment mappings for artifact movements")
    ...
    def clean_equipment_names(self):
        ...
        if not long_equipment.empty:
            ...
            for idx, row in long_equipment.iterrows():
                self.changes.append(f"Removing overly long equipment entry (ID: {row['EquipmentID']}): {row['Equipment'][:50]}...")
            
            # Remove from equipment table
            self.equipment = self.equipment[~self.equipment['EquipmentID'].isin(equipment_to_remove)]
            ...
            me_removed = me_before - len(self.me_map)
            if me_removed > 0:
                self.changes.append(f"Removed {me_removed} movement-equipment mappings for invalid equipment")
    ...
    def deduplicate_movements(self):
        ...
        if not duplicates.empty:
            ...
            for idx, row in duplicates.iterrows():
                self.changes.append(f"Removing duplicate movement: '{row['Movement']}' (ID: {row['MovementID']})")
            ...
            wm_removed = wm_before - len(self.wm_map)
            if wm_removed > 0:
                self.changes.append(f"Removed {wm_removed} workout-movement mappings for duplicate movements")
            
            me_before = len(self.me_map)
            self.me_map = self.me_map[~self.me_map['MovementID'].isin(duplicate_ids)]
            me_removed = me_before - len(self.me_map)
            if me_removed > 0:
                self.changes.append(f"Removed {me_removed} movement-equipment mappings for duplicate movements")
